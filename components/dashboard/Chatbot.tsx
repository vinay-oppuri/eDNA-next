...
import { Send, Bot, User, Code, BarChart3, Microscope, Sparkles, X, Loader2 } from 'lucide-react';
...
const suggestedPrompts = [
  ...
  { icon: <Microscope className="h-4 w-4 mr-2" />, text: "Compare taxonomic diversity between Arctic and Mediterranean samples" },
...
export function Chatbot({ showIntroMessage = true }: { showIntroMessage?: boolean }) {
  ...
  const handleSendMessage = (prompt?: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      userMessage,
      { id: 'typing', type: 'bot', content: '', timestamp: new Date(), isTyping: true }
    ]);
    setNewMessage('');
    scrollToBottom();
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-0 right-0 left-0 lg:left-auto lg:right-8 
                 bg-gray-900/80 backdrop-blur-lg border-t border-l border-r 
                 border-gray-700/50 rounded-t-2xl shadow-2xl z-50 overflow-hidden"
      style={{ width: 'calc(100% - 2rem)', maxWidth: '700px', margin: '0 auto' }}
    >
      {/* Header */}
      <header
            ...
            <div className="mt-6 space-y-2">
              {suggestedPrompts.map((prompt, i) => (
                ...
                >
                  <Button
                    variant="outline"
                    className="w-full justify-start h-auto p-3 bg-gray-800/50 border-gray-700 hover:bg-gray-700 text-gray-300"
