...
interface QueryResult {
  ...
  repSeqId: string;
  topDbMatch: string | null;
  functionSignal: string | null;
  abundance: string;
  envAssoc: string | null;
...
const mockQueryResults: QueryResult[] = [
  {
    ...
    // new fields
    taxConfidence: '96.2%',
    repSeqId: 'contig_1045',
    topDbMatch: 'Escherichia coli K12',
    functionSignal: 'Ribosomal RNA domain',
    abundance: '4.3%',
    envAssoc: 'Human gut',
    noveltyCategory: 'Known species',
    expertValidation: 'Reviewed ✅',
  },
  {
    id: 'Q002',
    sequence: 'GCTAGCTAGCTA...AGCT',
    similarity: 23.4,
    dbMatch: null,
    taxonomy: null,
    noveltyScore: 87.6,
    ...
    // new fields
    taxConfidence: '81.4%',
    repSeqId: 'contig_2056',
    topDbMatch: null,
    functionSignal: 'Unknown protein',
    abundance: '1.2%',
    envAssoc: 'Deep-sea vents',
    noveltyCategory: 'Novel genus candidate',
    expertValidation: 'Pending ⏳',
  },
  {
    id: 'Q003',
    sequence: 'TGCATGCATGCA...ATGC',
    similarity: 78.9,
    ...
    // new fields
    taxConfidence: '89.7%',
    repSeqId: 'contig_3077',
    topDbMatch: 'Bacteroidetes sp. clone A7',
    functionSignal: 'Polysaccharide degradation enzyme',
    abundance: '2.8%',
    envAssoc: 'Marine sediment',
    noveltyCategory: 'Novel species candidate',
    expertValidation: 'Reviewed ✅',
  },
];
...
export function QueryAnalysis() {
  ...
  const toggleAnalysis = (id: string) => {
    setSelectedAnalysis(prev => (prev === id ? null : id));
  };
  useEffect(() => {
    if (!isAnalyzing) {
      if (!isComplete) {
  ...
  const getResultIcon = (noveltyScore: number) => {
    if (noveltyScore > 70) return <AlertTriangle className="h-5 w-5 text-orange-500 dark:text-orange-400" />;
    if (noveltyScore < 30) return <CheckCircle className="h-5 w-5 text-green-500 dark:text-green-400" />;
    return <Dna className="h-5 w-5 text-blue-500 dark:text-blue-400" />;
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Enter a sequence or accession ID.</CardDescription>
        </CardHeader>
      ...
      {(isAnalyzing || isComplete) && (
        <Card className="mt-6 transition-all duration-500 ease-in-out">
          <CardHeader>
            <CardTitle className={`flex items-center space-x-3 transition-colors duration-500 ${isComplete ? 'text-green-500 dark:text-green-400' : ''}`}>
              ...
              <div className="space-y-8">
                {analysisSteps.map((step, index) => (
                  <div key={step.name} className="relative flex items-start">
                    ...
                    >
                      {isComplete ? (
                        <CheckCircle className={`h-6 w-6 text-green-500 dark:text-green-400 transition-all delay-300 duration-500`} style={{ transitionDelay: `${index * 100}ms` }} />
                      ) : step.status === 'completed' ? (
                      ...
                      <div className="mt-2 flex items-center space-x-3 text-sm text-muted-foreground">
                        <Progress
                          ...
                          className={`h-2 flex-1 [&>div]:bg-gradient-to-r
                            ${step.status === 'completed'
                              ? '[&>div]:from-green-400 [&>div]:to-green-600 dark:[&>div]:from-green-300 dark:[&>div]:to-green-500'
                      ...
                      <div className="flex space-x-2 pt-2">
                        ...
                        >
                          {selectedAnalysis === result.id ? (
                            <>
                              <ChevronUp className="h-4 w-4 mr-2" /> Hide Analysis
                            </>
                          ) : (
                            <>
                              <ChevronDown className="h-4 w-4 mr-2" /> See Analysis
                            </>
                          )}
                        </Button>
                        ...
                        <div className="flex flex-col pt-4 space-y-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
                            ...
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              ...
                              <span
                                className={`font-semibold ${result.similarity > 90
                            ...
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              ...
                              <span
                                className={`font-semibold ${result.noveltyScore > 70 ? 'text-red-500 dark:text-red-400' : 'text-amber-500 dark:text-amber-400'
                                  }`}
                              >
                                {result.noveltyScore}%
                              </span>
                            ...
                            {/* Tax Confidence */}
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              <span className="font-medium">Tax Confidence</span>
                              <span className="font-semibold text-indigo-500 dark:text-indigo-400">{result.taxConfidence}</span>
                            </div>

                            {/* Representative Sequence */}
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              <span className="font-medium">Representative Seq</span>
                              <span className="font-mono text-xs text-gray-700 dark:text-gray-300">{result.repSeqId}</span>
                            </div>

                            {/* Abundance */}
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              <span className="font-medium">Abundance</span>
                              <span className="font-semibold text-purple-500 dark:text-purple-400">{result.abundance}</span>
                            </div>

                            {/* Novelty Category */}
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              <span className="font-medium">Novelty Category</span>
                              <span className="font-semibold text-teal-500 dark:text-teal-400">{result.noveltyCategory}</span>
                            </div>

                            {/* Expert Validation */}
                            <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                              <span className="font-medium">Expert Validation</span>
                              <span className="font-semibold text-pink-500 dark:text-pink-400">{result.expertValidation}</span>
                            </div>
                            ...
                            {result.dbMatch && (
                              <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                ...
                                <span className="text-gray-600 dark:text-gray-300">{result.dbMatch}</span>
                              </div>
                            )}

                            {result.topDbMatch && (
                              <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                <span className="font-medium">Top DB Match</span>
                                <span className="text-gray-600 dark:text-gray-300">{result.topDbMatch}</span>
                              </div>
                            )}
                            ...
                            {result.taxonomy && (
                              <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                ...
                                <span className="text-gray-500 dark:text-gray-400 text-xs">{result.taxonomy}</span>
                              </div>
                            )}

                            {result.functionSignal && (
                              <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                <span className="font-medium">Function Signal</span>
                                <span className="text-gray-600 dark:text-gray-300">{result.functionSignal}</span>
                              </div>
                            )}

                            {result.envAssoc && (
                              <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                                <span className="font-medium">Environmental Association</span>
                                <span className="text-gray-600 dark:text-gray-300">{result.envAssoc}</span>
                              </div>
                            )}

                            {/* No database matches */}
                            {!result.dbMatch && !result.topDbMatch && (
                              <div className="col-span-full p-4 bg-amber-50 border-l-4 border-amber-400 dark:bg-amber-900/50 dark:border-amber-600 rounded-md">
                                <p className="text-amber-800 dark:text-amber-300 text-sm flex items-center gap-1">
                                  <AlertTriangle className="h-4 w-4" />
                                  No significant database matches found.
                                </p>
                              </div>
                            )}
                          </div>
